...
\usepackage{doi} %ensures references have DOI
...
%     Front‑matter
% -----------------------------------------------------------------------------
\title[Mathematical Signatures of Malignancy: Forecasting Glioblastoma Multiforme By Implementing Graph Algorithms in Gene Coexpression Networks]{Mathematical Signatures of Malignancy: Forecasting Glioblastoma Multiforme By Implementing Graph Algorithms in Gene Coexpression Networks}
...
% -----------------------------------------------------------------------------
\begin{abstract}
    Traditionally, genomic analyses rely on examining key genes that when mutated greatly increase the risk of malignancy. However, there are a multitude of cases where cancer was formed despite not having the genes identified to have the greatest risk of malignancy. This necessitates a novel form of genetic screening that takes into account the whole rather than the part. Graph Theory was therefore posited as a holistic cancer screening tool, where genes are the nodes, the edges are the interactions between the genes, and the Pearson's Correlation Coefficient, if $p\text{-value} < \alpha \hspace{0.5em} \text{(where } \alpha = 0.05\text{)}$, are the edge weights in what is known as a gene coexpression analysis. A comparative analysis was done between cancer and normal networks, consisting of 4 key mathematical signatures that can be expanded upon. These are degree distribution and power-law exponent, clique cover number and Ramsey theory, k-core decomposition, and spectral analysis using Laplacian eigenvalue spectrum. All of these signatures reveal unique graphical properties which have numerous implications on the etiology of cancer. In this study, the 4 mathematical signatures were validated for efficacy and the p-values examined for significance. Additional topological data as well as histogram figures were examined to further investigate the advent of the 4 tested mathematical signatures. Our results confirmed significant differences, and in future we can look at how these differences vary in other varieties of cancer in a meta-analysis. 
  %An undirected graph $G_{m,n}$ is defined on the vertex set $V = I_n = \set{1,2,3,\dots,n}$ where $u,v\in V$ are adjacent \textit{iff} $u\neq v$ and $u+v$ is not divisible by $m$, with $m>1$.  For a graph $G=(V,E)$, a subset $S\subseteq V$ is called a \emph{dominating set} if every vertex $a\in V$ is either in $S$ or is adjacent to a vertex in~$S$.  The \emph{domatic number} $d(G)$ of $G$ is the maximum integer $k$ such that $V$ can be partitioned into $k$ pairwise disjoint dominating sets $D_1,\dots,D_k$. Such a partition is called a \emph{domatic partition}. We present several results on domination parameters of the graph $G_{m,n}$, deriving bounds for its domatic number for various values of $m$ and~$n$.
\end{abstract}
...
\[
  C = \biggl(\sum_{k=k_{\min}}^{\infty} k^{-\alpha}\biggr)^{-1}.  \label{eq:normalization}
\]
...
\begin{enumerate}
  \item Fit $\widehat{\alpha}$ and record the Kolmogorov--Smirnov statistic $D_{\rm obs}$.
